...
class Potential():
    ...
    def evol(self, t):
      ...
      Args
       t: float, time
       uext: function, the form of the potential
      Returns:
...
class ConstPot(Potential):
   ...
   def __init__(self, amplitude, grid):
      n1, n2, n3 = grid
      self.form = lambda t: amplitude * torch.ones(n1,n2,n3, dtype=torch.cdouble)

class RampPot(Potential):
   """Creates a ramp potential that evolves like
   initial + t * (final - initial) 
   """
   def __init__(self, initial, final, grid):
      n1, n2, n3 = grid
      self.form = lambda t: initial + (final - initial) * t * torch.ones(n1,n2,n3, dtype=torch.cdouble)

class HarmonicPot(Potential):
   def __init__(self, grid, x_min, dx, w, amplitude=1):
      n1, n2, n3 = grid
      # Build space and momentum grids
      x1 = x_min[0] + torch.arange(n1, dtype=torch.float64)*dx[0] # size n1
      x2 = x_min[1] + torch.arange(n2, dtype=torch.float64)*dx[1]
      x3 = x_min[2] + torch.arange(n3, dtype=torch.float64)*dx[2]
      gx, gy, gz = torch.meshgrid(x1, x2, x3)
      self.form = lambda t: 0.5 * amplitude * ((w[0]*gx)**2 + (w[1]*gy)**2 + (w[2]*gz)**2)
